  ...
  module Tools
    class Nut
      ...
      def report_ups_status(ups)
        ...
        ups_state = "ok" if /\b(OL)\b/.match?(upsc[ups]["ups.status"])
        ups_state = "warning" if /\b(OB|DISCHRG|BYPASS)\b/.match?(upsc[ups]["ups.status"])
        ups_state = "critical" if /\b(ALARM|OVER)\b/.match?(upsc[ups]["ups.status"])
